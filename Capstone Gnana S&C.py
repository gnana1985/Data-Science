...
### Finding top 3 common venue categories in Toronto


# In[77]:


t_venue_cat_common=toronto_venues.groupby('Venue Category').count().sort_values('Venue', ascending=False)
t_venue_cat_common.head(3)


# In[79]:


toronto_cs_venues=toronto_venues[toronto_venues['Venue Category'].str.contains("Coffee Shop")].reset_index(drop=True)
toronto_top_cs_venues=toronto_cs_venues.groupby('Neighborhood', as_index=False).count().sort_values('Venue', ascending=False).head(10)
toronto_top_cs_venues["Neighborhood"]
...
y=toronto_top_cs_venues.iloc[0:,0]
y
...
toronto_top_cs_n = pd.merge(df_post,y,how='left',left_on='Neighbourhood',right_on='Neighborhood')
toronto_top_cs_n.drop('Neighbourhood',axis=1,inplace=True)
toronto_top_cs_n
