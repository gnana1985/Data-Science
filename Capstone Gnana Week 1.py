...
# This project is aimed at finding an optimal neighborhood location for a restaurant. Specifically, this report is to advice a Thain restaurant chain that is interested in opening an outlet in Toronto.
# 
...
# 
# 1. Neighborhood details for Toronto. Web crawling to understand the Toronto neighbourhood preferences, demographics and its food industry. https://en.wikipedia.org/wiki/List_of_postal_codes_of_Canada:_M, will be scraped using the sing the BeautifulSoup packageto get the basic neighborhood data. This will be augmented by geographical coordinates using the data in the csv file : http://cocl.us/Geospatial_data
# 
# 2. Venue details that includes Thai and non Thai restaurants. For this Foursqaure API Venues data will be used.
# 
# Data analysis:
# 1. Basic clean up of the datasets will be performed to make it suitable for running any data science analytics
# 2. Neighborhoods will be clustered and  analysed to understand the spending patterns and restaurant density in general. Geographical proximity and its association with the demographics of the physical locations will be analyzed which will provide key pointers while recommending the loaction for Thai restaurants
...
# ##### Cluster0 seems like a zone inhabited with health concious poeple.
...
# ##### Cluster2 seems to have venues particularly suited for a peaceful community and large greenspace in cenrtal Toronto and seems prosperous.
...
# ##### Cluster3 seems to have venues that are popular in the busy areas as is probably marks the business district of Toronto with lots of cafes, pubs and light weight entertainment centers to suit the busy lifestyle of the working class.
...
# ##### Cluster4 seems to have venues characteristic of people who want to enjoy peaceful and leisure living. 
# Is a residential locality surrounded by posh neigbourhoods. Is geographically toward the north, away from the key business district.
...
# On the basis of the visual inspection of the existing thai restaurants in Cluster3, 
# any new restaurant can be positioned in the western Cluster3 neighborhoods ie in the area between High Park and Salad King which shares the characteristics of neighborhoods in which the restaurant business is thriving.
# Top picks are the neighborhoods of Dovercourt Village and Christie given the low restaurant density in that, competition is less and hence can prove to be quite profitable.
# 
